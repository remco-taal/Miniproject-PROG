...
def start():
    ...
    if 'error' not in dictionary:  # De XML bevat een dictionary error wanneer een verkeerde waarde wordt ingevuld
        ...
        for tijd in dictionary['ActueleVertrekTijden']['VertrekkendeTrein']:
            ...
            label1 = Label(root,text='Het begin station is: Utrecht {:9} {} uur De eindbestemming is: {}'.format(treinSoort, vertrekTijd,eindbestemming)).place(x=1000, y=40 + (spacing * amount))
            amount +=1
    else:
