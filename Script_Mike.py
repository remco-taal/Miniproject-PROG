...
def dothis():
    ...
    if 'error' not in dictionary:  # De XML bevat een dictionary error wanneer een verkeerde waarde wordt ingevuld
        ...
        for tijd in dictionary['ActueleVertrekTijden']['VertrekkendeTrein']:
            ...
            label1 = Label(root,text='Het begin station is: {} {:9} {} uur De eindbestemming is: {}'.format(station.capitalize(),treinSoort, vertrekTijd,eindbestemming)).place(x=350, y=40 + (spacing * amount))
            amount +=1
    else:
...
button1 = Button(root, text="Ik wil naar\n  Amsterdam       ",fg="white", bg="blue",command = dothis)
